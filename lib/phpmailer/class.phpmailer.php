...
class PHPMailer {
     ...
     * @return void
     */
    public function Init($sender, $rcpt, $content_arr, $debug = 0) {
        ...
        $this->SMTPSecure = @$sender['secure']!="" ? $sender['secure'] : "ssl";
        ...
        foreach($rcpt as $r):
            $this->addAddress($r);
        endforeach;
        $this->addReplyTo($sender['email'], $sender['name']);

        if($content_arr['attachment'] != "")
            $this->addAttachment($content_arr['attachment']);

        $this->isHTML(true);
        $this->Subject = $content_arr['subject'];
        $this->Body    = $content_arr['html'];
        $this->AltBody = $content_arr['text'];
    }
