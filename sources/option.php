<?php
switch (true) {
	case is_array($option) && intval($option['uri_enable'])==1:
		unset($_SESSION['index_product_filter'][$option['type']]);
		$uns = false;
		$type = "brand_id";
		$id_cat = $option['id'];
         
         if (isset($_SESSION['brand_id'])) {
             if ($_SESSION['brand_id'] != $option['id'] ) {
             	unset($_SESSION['brand']);
             }
        }
 

		if (@$_SESSION['index_product_filter']['category'] != "") {
		...
		if($_POST['loadProduct'] == "1") {
			...
			$limit *= 2;
			$row_product = getItems(array("table" => "product", "condition" => "where {$where} and {$where_translate} and find_in_set({$option['id']}, {$option['type']}_id) and enable>0 order by level desc, create_date desc, title limit {$start}, {$limit}"));
			$data = array();
