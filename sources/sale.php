...
if($_POST['loadProduct'] == "1") {
	...
	$limit *= 2;
	$row_product = getItems(array("table" => "product", "condition" => "where {$where} and {$where_translate} and popular3>0 and enable>0 order by popular3, level desc, create_date desc, title limit {$start}, {$limit}"));
	$data = array();
