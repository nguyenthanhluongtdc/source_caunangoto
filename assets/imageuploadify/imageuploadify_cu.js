  ...
  $.fn.imageuploadify = function(opts) {
    ...
    this.each(function() {
      ...
      const retrieveFiles = (files) => {
        ...
        $(self).data("quantity", totalFiles.length);
      ...
      const readingFile = (id, file) => {
        ...
        if (file.type && file.type.search(/image/) != -1) {
          ...
          fReader.onloadend = function (e) {
            ...
            contentList.push(e.target.result);
      ...
      $(self).closest("form").on("submit", function(event) {
        ...
        let hidden = $(`input[name="${$(self).data("name")}"]`);
        let content = [];
        if(hidden.attr("type") == undefined) {
          hidden = $(`<input type="hidden" name="${$(self).data("name")}" value=""`);
          $(this).append(hidden);
        }
        for(key in contentList) {
          if(contentList[key] != "")
            content.push(contentList[key]);
        }
        $(hidden).val(content.join(";;"));
        return true;
      });
