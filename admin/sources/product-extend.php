...
$product = getItems(array("table" => "product", "id" => $_REQUEST['product_id']));

if(is_array(@$product)) {
	$r_tmp = getTranslate("product", $row_language[0]['id'], $product['id']);
...
switch (true) {
	case $act=="list":
		$items = getItems(array("table" => $table, "condition" => "where type like '{$_GET['type']}' and product_id like '{$product['id']}'", "limit" => 20, "pagination" => true));
		$paging = $pagination->getSource("&p");
		$template = "product-extend/list";
		break;
	case $act=="edit":
		if($_REQUEST['id'] != "") {
	...
	case $act=="save":
		if(isset($_POST['savebtn'])) {
			...
			$data = array("title", "product_id", "thumbnail", "popular", "enable", "index", "type");
