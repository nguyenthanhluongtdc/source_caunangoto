<div class="well"><?= $_GET['id']!="" ? "Cập nhật" : "Thêm" ?> <?= $attribute_enable[$_GET['type']]['title'] ?> mở rộng <?= @$item['title']!="" ? "'".$item['title']."'" : (@$item['thumbnail']!="" ? "<img src='".getThumbnailUrl($item['thumbnail'])."' style='max-width: unset !important; height:40px !important;'>" : "") ?> (danh mục '<?= !empty($category) ? implode(" ", array_slice(explode(" ", $category['title']), 0, 3)) . (substr_count($category['title'], " ")>2 ? "..." : "") : "" ?>')
</div>
